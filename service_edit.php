...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($provider_name==='' || $name===''||$company_name===''||$company_place===''||$company_contact===''||$issue_found===''||$solution==='') {
    ...
    $st = $pdo->prepare("UPDATE services SET
      ...
      machine_number=?, machine_status=?, spares_used=?,
  ...
  <form method="post" class="grid" enctype="multipart/form-data">
    ...
    <div class="col-6">
        ...
        <select name="machine_status">
            <?php 
            $statuses = ['Out of Warranty', 'Under Warranty', 'AMC'];
            foreach($statuses as $s): 
            ?>
            <option value="<?php echo $s; ?>" <?php echo (isset($r['machine_status']) && $r['machine_status'] === $s) ? 'selected' : ''; ?>>
                <?php echo $s; ?>
            </option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="col-6">
        ...
        <input name="spares_used" value="<?php echo isset($r['spares_used']) ? h($r['spares_used']) : ''; ?>" placeholder="List spares used..." />
    </div>
